...
Le programme est complexe mais on peux analyser qu'il s'agit d'une boucle infini contenant plusieurs fonctions que l'on connait bien. Essayons de comprendre un peu plus les variables que le binaire peux contenir.
...
On remarque qu'en contenu de variable, on trouve `auth` et `service`.
On peux comprendre alors qu'il contient plusieurs comparaison de variables associées à des mots logiques pour une authentification et on va donc essayer de les trouver et de les traduire avec gdb.
